...
// .wrangler/tmp/bundle-qOjUuP/checked-fetch.js
...
// .wrangler/tmp/bundle-qOjUuP/strip-cf-connecting-ip-header.js
...
// .wrangler/tmp/bundle-qOjUuP/middleware-loader.entry.ts
init_checked_fetch();
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();

// .wrangler/tmp/bundle-qOjUuP/middleware-insertion-facade.js
init_checked_fetch();
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
...
var StorageFileApi = class {
  ...
  uploadOrUpdate(method, path, fileBody, fileOptions) {
    return __awaiter4(this, void 0, void 0, function* () {
      try {
        ...
        const data = yield (method == "PUT" ? put : post)(this.fetch, `${this.url}/object/${_path2}`, body, Object.assign({ headers }, (options === null || options === void 0 ? void 0 : options.duplex) ? { duplex: options.duplex } : {}));
        return {
  ...
  createSignedUrl(path, expiresIn, options) {
    return __awaiter4(this, void 0, void 0, function* () {
      try {
        let _path2 = this._getFinalPath(path);
        let data = yield post(this.fetch, `${this.url}/object/sign/${_path2}`, Object.assign({ expiresIn }, (options === null || options === void 0 ? void 0 : options.transform) ? { transform: options.transform } : {}), { headers: this.headers });
...
var CreateArtistSchema = z.object({
...
app.get("/api/artists", async (c) => {
  ...
  try {
    const { data, error } = await supabase.from("artist_profiles").select("*").eq("user_id", user.sub).order("created_at", { ascending: false });
    if (error)
      throw error;
    return c.json(data);
  } catch (error) {
...
app.get("/api/artists/:id", async (c) => {
  ...
  try {
    const { data, error } = await supabase.from("artist_profiles").select("*").eq("id", id).single();
    if (error)
      throw error;
    return c.json(data);
  } catch (error) {
...
app.get("/api/releases", async (c) => {
  const supabase = c.get("supabase");
  try {
    const { data, error } = await supabase.from("music_releases").select(`
        *,
        artist_profiles (
          artist_name,
          profile_image_url
        )
      `).eq("status", "released").order("release_date", { ascending: false });
    if (error)
      throw error;
    return c.json(data);
  } catch (error) {
...
app.post("/api/releases", zValidator("json", CreateReleaseSchema), async (c) => {
  ...
  try {
    const { data, error } = await supabase.from("music_releases").insert(releaseData).select().single();
    if (error)
      throw error;
    return c.json(data, 201);
  } catch (error) {
...
app.get("/api/analytics", async (c) => {
  ...
  try {
    let query = supabase.from("fan_analytics").select("*").eq("artist_id", artistId).order("period_start", { ascending: false });
    if (platform) {
...
app.get("/api/calendar", async (c) => {
  ...
  try {
    const { data, error } = await supabase.from("content_calendar").select("*").eq("artist_id", artistId).order("scheduled_date", { ascending: true });
    if (error)
      throw error;
    return c.json(data);
  } catch (error) {
...
app.post("/api/calendar", zValidator("json", CreateCalendarSchema), async (c) => {
  ...
  try {
    const { data, error } = await supabase.from("content_calendar").insert(calendarData).select().single();
    if (error)
      throw error;
    return c.json(data, 201);
  } catch (error) {
